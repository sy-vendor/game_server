...
-export([get_node_id/0, get_node/0, add_node/6, del_node/1, hide_node/1, show_node/1, get_kfclient_node/0, rpc_cast_logic/3, rpc_cast_other/3,
  get_other_logic_node/0, i/0, p/0, call/1, cast/1, get_all_node/0, get_logic_node/0]).
...
%% @doc 获取跨服通信节点名称[0线节点名]
get_kfclient_node() ->
  case ets:lookup(?ETS_NODE, ?NODE_ID_KFCLIENT) of
    [Node] ->
      Node#node.name;
    _ ->
      case get_node_id() =:= ?NODE_ID_KFCLIENT of
        true -> node();
        false -> none
      end
  end.
...
get_logic_node() ->
  ...
  [Node || #node{id = Id} = Node <- NodeList, Id >= ?NODE_ID_GAME, Id < ?NODE_ID_KFCENTER].
