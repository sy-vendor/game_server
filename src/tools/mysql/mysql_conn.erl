...
-include("mysql.hrl").
-define(mysql_socket_closed, "mysql_recv: socket was closed").
-record(state, {
	  mysql_version,
	  log_fun,
	  recv_pid,
...
loop(State) ->
    ...
    receive
	{fetch, Queries, From} ->
        %% io:format("send_msg_1\n", []),
        %% case sys_env:get(mysql_sleep) of
        %%     Timeout when is_integer(Timeout) ->
        %%         util_proc:sleep(Timeout);
        %%     _ ->
        %%         ignore
        %% end,
        %% io:format("send_msg_2\n", []),
        case do_queries(State, Queries) of
            Reply = {error, ?mysql_socket_closed} ->
                send_reply(From, Reply),
                error;
            Other ->
                send_reply(From, Other),
                loop(State)
        end;
	...
	{mysql_recv, RecvPid, data, Packet, Num} ->
	    ...
	    loop(State);
    close ->
	    State#state.recv_pid ! stop,
        stop;
    {print} ->
        io:format("[mysql] state: ~w\n", [State]),
        loop(State);
    Unknown ->
        io:format("[mysql] recv: ~w\n", [Unknown]),
	    ?Log2(LogFun, error,
		  "received unknown signal, exiting: ~p", [Unknown]),
	    error
    end.
...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
	...
	ok ->
	    get_query_response(LogFun,RecvPid,
				    Version);
	{error, Reason} ->
        io:format("[mysql] Failed sending data on socket : ~w", [Reason]),
	    {error, ?mysql_socket_closed}
    end.
