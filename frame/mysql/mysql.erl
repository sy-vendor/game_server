...
%% 丢弃旧链接，建立新链接
connect_new(PollId, Host, Port, User, Password, Database, Encoding, Reconnect) ->
    case gen_server:call(?SERVER, {clean_conn_poll, PollId}) of
        ok ->
            connect(PollId, Host, Port, User, Password, Database, Encoding, Reconnect);
        {false, Reason} ->
            {false, Reason}
    end.
...
print(PoolId) ->
    gen_server:call(?SERVER, {print, PoolId}).
...
handle_call({clean_conn_poll, PollId}, _From, State) ->
    PidList = gb_trees:to_list(State#state.pids_pools),
    Fun = fun({Pid, Poll}, Accu) when Poll =:= PollId ->
        case remove_conn(Pid, Accu) of
            {ok, _, NewAccu} ->
                Pid ! close,
                NewAccu;
            error ->
    ...
    NewState = lists:foldl(Fun, State, PidList),
    {reply, ok, NewState};
...
handle_call({print, PoolId}, _From, State) ->
    ...
    case gb_trees:lookup(PoolId, ConnPools) of
        ...
        {value, {Unused, Used}} ->
            lists:foreach(fun(_Conn = #conn{pid = Pid}) ->
                        io:format("conn: ~w\n", [Pid]),
                        Pid ! {print}
                end, Unused ++ Used)
    end,
